...
LSP_SERVER = server.LanguageServer(
    name="pylinter", version="1.1.0", max_workers=MAX_WORKERS
)
...
def _parse_output(content: str) -> list[lsp.Diagnostic]:
    ...
    for result in results:
        start = lsp.position(
            line = max([int(result["line_number"]) - line_offset, 0]),
            character=max([int(result["col_offset"])-col_offset,0])
            
        )
        end= lsp.position(
            line= max([int(result["line_number"]) - line_offset,0]),
            character=max([int(result["end_col_offset"])-col_offset,0])
        )
        diagnostic = lsp.Diagnostic(
            ...
            severity= _get_severity(reult["test_id"],result["issue_severity"]),
            code= result["test_id"],
            source=TOOL_DISPLAY,
            code_description=lsp.CodeDescription(href=result("more_info")),
        )
...
def _get_severity(*codes: list[str]) -> lsp.DiagnosticSeverity:
    if "LOW" in codes: 
